...
async function api(url,opt={}){opt.headers={...(opt.headers||{}),Authorization:'Bearer '+token};if(opt.body&&!opt.headers['Content-Type']&&!(opt.body instanceof FormData))opt.headers['Content-Type']='application/json';const mutation = opt.method === 'POST' && /^\/api\/(receive|issue|transfer|stock-count|adjustments)(\/|$)/.test(url); const retrySlot = 'easywms_retry:' + (me?.id || '') + ':' + url; if(mutation){const saved=JSON.parse(sessionStorage.getItem(retrySlot)||'null');const pending=saved&&saved.body===opt.body?saved:{body:opt.body,key:crypto.randomUUID()};sessionStorage.setItem(retrySlot,JSON.stringify(pending));opt.headers['Idempotency-Key']=pending.key;} const r=await fetch(url,opt);if(mutation&&(r.ok||(r.status>=400&&r.status<500)))sessionStorage.removeItem(retrySlot);if(r.status===401){logout();throw new Error(validationMessage('session expired'))}const ct=r.headers.get('content-type')||'';const d=ct.includes('application/json')?await r.json():await r.blob();if(!r.ok)throw new Error(validationMessage(d.error||'request failed'));return d}
